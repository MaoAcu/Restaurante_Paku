...
def GetMenu():
    try:
        ...
        menus = Menu.query.filter_by(local=local_id).all()
...
def PatchMenuSection(idmenu):
    try:
        menu = Menu.query.filter_by(idmenu=idmenu, local=5).first()
        if not menu:
...
def DeleteMenu(idmenu):
    try:
        ...
        if menu.imagen:
            image_path = os.path.join(current_app.root_path, "static", "images","menu", menu.imagen)
